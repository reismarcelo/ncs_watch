...
from .commands import apply_cmd, schema_cmd
from . import app_config


logger = logging.getLogger('ncs_watch.main')
...
def main():
    ...
    cli_parser.add_argument("--version", action="version", version=f"NCS Watch Version {version}")
    ...
    apply_parser.add_argument('-u', '--user', metavar='<user>', action=EnvVar, required=False,
                              envvar='NCS_WATCH_USER', type=non_empty_type,
                              help='username, can also be defined via NCS_WATCH_USER environment variable. '
    ...
    apply_parser.add_argument('-p', '--password', metavar='<password>', action=EnvVar, required=False,
                              envvar='NCS_WATCH_PASSWORD', type=non_empty_type,
                              help='password, can also be defined via NCS_WATCH_PASSWORD environment variable. '
    ...
    apply_parser.add_argument("-s", "--save", metavar="<filename>", type=non_existing_file_type,
                              default=f"periodic_{datetime.now():%Y%m%d_%H%M%S}.zip",
